...
getAttribute str (Element _ as xs)
  | val /= Nothing = fromMaybe "" val
  | otherwise      = head $ dropWhile (== "") (map (getAttribute str) xs)
  where 
    val = lookup str as
getAttribute _ _
  = ""
...
getChildren str (Element name _ xs)
  = filter (\xml -> checkName xml) xs
  where
    checkName (Element n _ _)
      = n == str  
    checkName _ 
      = False  
getChildren _ _
...
addChild xml (Element name as xs)
  = Element name as (addEnd xml xs)
  where 
    addEnd x = reverse . (x :) . reverse    
...
parseAttributes str
  = parseA' $ skipSpace str
  where 
    parseA' :: String -> (Attributes, String)
    parseA' ('>' : ss)
      = ([],  ss)
    parseA' str@(s : ss)   
      = ((name, val) : as, rest'')
      where
        ...
        (as, rest'') = (parseA'. skipSpace .  tail) rest' --tail rest removes the closing '\"'
...
parse' "" ((Element _ _ (x : _)) : _)
  = x
parse' ('<' : '/' : ss) st
  = parse' str' (popAndAdd st)
  where
    (_, str) = break (== '>') ss
    str'     = tail str        -- removes the leading '>'
parse' ('<' : ss) st
  = parse' rest' (Element eName as [] : st)
  where
    (eName, rest) = parseName ss 
    (as, rest')   = parseAttributes rest
parse' ss st 
  = parse' ss' (addText text st) 
  where
    (text, ss') = break (== '<') ss
-------------------------------------------------------------------------
-- Part III
