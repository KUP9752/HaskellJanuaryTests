...
updateVar (id, val) st
  | null st'  = (id, (Local, val)) : st
  | otherwise = (id, (sc, val)) : st''
  where
    st'            = filter ((== id) . fst ) st
    (_, (sc, _))   = head st'
    st''           = filter ((/= id) . fst ) st
...
applyOp Index (A a) (I i)
  | null val  = I 0
  | otherwise = I (head val)
  where
    val = [y | (x, y) <- a, x == i]

-- Helper for Ord functions
boolConv :: Bool -> Value
boolConv False = I 0
boolConv _     = I 1
...
eval (Var id) _ st
  = getValue id st
eval (OpApp op e e') fds st
  = applyOp op (eval e fds st) (eval e' fds st)
eval (Cond p q r) fds st
  | evalP     = eval q fds st
  | otherwise = eval r fds st
  where
    evalP = evalBool $ eval p fds st
...
-- Helper for converting values to Bool values for guards
evalBool :: Value -> Bool
evalBool (I 0) = False
evalBool _     = True
...
executeStatement (Assign id e) fds _ st
  = updateVar (id, eval e fds st) st
executeStatement (AssignA id e e') fds _ st
  = updateVar (id, assignArray (getValue id st) (eval e fds st) (eval e' fds st)) st
executeStatement (If e b b') fds pds st
  | evalE     = executeBlock b fds pds st
  | otherwise = executeBlock b' fds pds st
  where
    evalE = evalBool $ eval e fds st
executeStatement (While e b) fds pds st
  | evalE     = executeStatement (While e b) fds pds (executeBlock b fds pds st)
  | otherwise = st
  where
    evalE = evalBool $ eval e fds st
executeStatement (Call id p es) fds pds st
  | id == ""  = retSt
  | otherwise = updateVar (id, getValue "$res" st') retSt 
  where
    (as, b) = lookUp p pds    
    bs      = bindArgs as (evalArgs es fds st)
    st'     = executeBlock b fds pds (bs ++ st)
    globs   = getGlobals st'
    locs    = getLocals st
    retSt   = locs ++ globs
executeStatement (Return e) fds pds st
...
executeBlock [] _ _ st
  = st
executeBlock (s : ss) fds pds st
  = executeBlock ss fds pds (executeStatement s fds pds st)
